...
async function main(prompt = '') {
  ...
  while (true) {
    // Map MESSAGE_DB to Gemini API format (role: user/model)
    const contents = MESSAGE_DB.map(msg => ({
    ...
    try {
      ...
      // Add a small delay between steps to reduce hitting rate limits proactively
